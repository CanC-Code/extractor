...
// Standalone LZ4 block decompression routine
...
uint32_t bswap32(uint32_t val) { return (val >> 24) | ((val >> 8) & 0x0000FF00) | ((val << 8) & 0x00FF0000) | (val << 24); }
uint64_t bswap64(uint64_t val) { return ((uint64_t)bswap32(val & 0xFFFFFFFF) << 32) | bswap32(val >> 32); }
...
size_t parse_unityfs_blob(uint8_t* buffer, size_t max_size) {
    ...
    for (uint32_t i = 0; i < nodesCount; i++) {
        ...
        std::cout << "[C++ Engine] Node Extracted: " << nodePath << " (" << nodeSize << " bytes)" << std::endl;
        
        // Pass the extracted file back to the JS DOM Environment dynamically.
        EM_ASM({
            var fileName = UTF8ToString($0);
            if (typeof window.onFileExtracted === 'function') {
                window.onFileExtracted(fileName, $1, $2);
            } else {
...
extern "C" {
    // Upgraded memory scanner to support APK container analysis natively
    void process_unity_archive(uint8_t* buffer, size_t size) {
        ...
        while (offset <= size - 8) {
            ...
            if (buffer[offset] == 'U' && std::memcmp(buffer + offset, "UnityFS", 7) == 0) {
                ...
                if (parsed_bytes > 0) {
                    offset += parsed_bytes; // Jump ahead to avoid redundant internal scanning
                } else {
                    offset += 8;
                }
            } else {
        ...
        if (!archive_found) {
            std::cout << "[C++ Engine] VALIDATION FAILED: No UnityFS signatures detected in provided APK." << std::endl;
        } else {
